...
To create custom components is to create components that accepts variety, uses abstraction. Using the power of JS, we can have a function that, depending on the case, can return dynamic content. For the exercise we created a message function that accepts an object, which can be modified to get a div tag as a JSX, which we can use to get dynamic content.

But that is not yet a proper custom element as it more of an interpolation technique. To make a more appropriate custom element as seen in extra credit 1, we pass the message as a type, in `React.createReactElement()` which is the first argument, and then passing the children object as a parameter, solves it. The babel compiler knows exactly what to create.

For the most proper custom element, we need JSX, since we dont want to use raw React APIs. For that we just need to create a custom tag, in this case `<Message>`, notice the capital case, otherwise it will be mixed with regular tags available in the DOM API. So uppercasing is actually a syntax for JSX to use a custom component.

...
createElement and getElementById is all we need to solve this exercise as it wanted us to create some HTML element without writing it explicitly via HTML. The browser have a DOM API that we can modify at runtime using Javascript and exploiting that we can create dynamic content without ever touching the HTML stuff.

Here we created a root element by hand in HTML and appended a `div` to it using JS, with the content of the div set to our very own "Hello World"

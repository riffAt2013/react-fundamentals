...
We are using JSX for the first time rather than going for raw React APIs. Since JSX is not javascript the browser doesn't understand what it is, so we need a compiler that compiles JSX to JS. We use `babel` for that. Babel can also be scripted as a CDN with the ability to use a specific MIME type for the script tag to work, `text/babel`.

Using that power we write JSX to write declarative React code that is more understandable and manipulate the power of HTML embedded in JS. We can use a special property of the objects, spread property to write even more concise UI codes. Such as creating an object with the className and children property can be embedded into a self closing div with the spread operator on the object. This will make Babel turn the code into proper React code with proper `createReactElement` attributes.  
